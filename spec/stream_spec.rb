...
describe LaunchDarkly::StreamProcessor do
  ...
  let(:processor) { subject.new("sdk_key") }
  ...
  describe '#process_message' do
    ...
    it "will accept PUT methods" do
      ...
      expect(processor.instance_variable_get(:@config).feature_store.get(LaunchDarkly::FEATURES, "asdf")).to eq(key: "asdf")
      expect(processor.instance_variable_get(:@config).feature_store.get(LaunchDarkly::SEGMENTS, "segkey")).to eq(key: "segkey")
    end
    it "will accept PATCH methods for flags" do
      processor.send(:process_message, patch_flag_message)
      expect(processor.instance_variable_get(:@config).feature_store.get(LaunchDarkly::FEATURES, "asdf")).to eq(key: "asdf", version: 1)
    end
    it "will accept PATCH methods for segments" do
      processor.send(:process_message, patch_seg_message)
      expect(processor.instance_variable_get(:@config).feature_store.get(LaunchDarkly::SEGMENTS, "asdf")).to eq(key: "asdf", version: 1)
    end
    it "will accept DELETE methods for flags" do
      processor.send(:process_message, patch_flag_message)
      processor.send(:process_message, delete_flag_message)
      expect(processor.instance_variable_get(:@config).feature_store.get(LaunchDarkly::FEATURES, "key")).to eq(nil)
    end
    it "will accept DELETE methods for segments" do
      processor.send(:process_message, patch_seg_message)
      processor.send(:process_message, delete_seg_message)
      expect(processor.instance_variable_get(:@config).feature_store.get(LaunchDarkly::SEGMENTS, "key")).to eq(nil)
    end
